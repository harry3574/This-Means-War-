...
def get_profile_details(profile_id):
    """Get detailed profile information with COALESCE for NULL values"""
    with sqlite3.connect(DATABASE_FILE) as conn:
        cursor = conn.cursor()
        cursor.execute("""
            SELECT p.name, p.emoji, 
                   COALESCE(MAX(s.current_war), 1), 
                   COALESCE(MAX(s.current_skirmish), 0)
            FROM profiles p
...
def get_all_profiles():
    """Get all profiles with their current progress and proper defaults"""
    with sqlite3.connect(DATABASE_FILE) as conn:
        cursor = conn.cursor()
        cursor.execute("""
            SELECT 
                ...
                p.name, 
                p.emoji, 
                COALESCE(MAX(s.current_war), 1) AS current_war,
                COALESCE(MAX(s.current_skirmish), 1) AS current_skirmish
            FROM profiles p
            LEFT JOIN saves s ON p.id = s.profile_id
            GROUP BY p.id
        """)
